    ...
    ?>
        <a href="#" class="list-group-item">
            <div class="row g-0 align-items-center">
                ...
                <div class="col-10">
                    <div class="text-dark">Заявка номер <?= $rowss['id'] ?> е приключена</div>
    ...
    ?>
        <a href="#" class="list-group-item">
            <div class="row g-0 align-items-center">
                ...
                <div class="col-10">
                    <div class="text-dark">Заявка номер <?= $rowss['id'] ?> е стартирана</div>
                    <div class="text-muted small mt-1">Име на клиент: <?= $rowss['customerName'] ?></div>
                    <div class="text-muted small mt-1">
                        <?php
                        $datetimeago = date("Y-m-d H:i:s");
...
$query2 = "SELECT * FROM productrequest WHERE view = '1'";
$query_run2 = mysqli_query($con, $query2);


while ($rowsss = mysqli_fetch_array($query_run2)) {
    ?>
    <div class="list-group-item">
        <button value="<?= $rowsss['id'] ?>" class="x-icon btn-def"><i class="bi bi-x-lg"></i></button>
        <input type="hidden" value="<?= $rowsss['id'] ?>">
        <div class="row g-0 align-items-center">
            <div class="col-2">
                <i class="text-primary bg-gradient bi bi-people fa-2x"></i>
            </div>
            <div class="col-10">
                <div class="text-dark">Заявка от екип: <?= $rowsss['teamName'] ?></div>
                <div class="text-muted small mt-1">Продукт: <?= $rowsss['name'] ?> <?= $rowsss['quantity'] . ' бр.' ?></div>
                <div class="text-muted small mt-1">
                    <?php
                    $datetimeago = date("Y-m-d H:i:s");
                    $diff = strtotime($datetimeago) - strtotime($rowsss['date']);
                    $sec = $diff;
                    $min = round($diff / 60);
                    $hrs = round($diff / 3600);
                    $days = round($diff / 86400);
