...
$(document).on('submit', '#productRequestForm', function(e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (res.status == 200) {
                ...
                $('#requestProdiuctModal').modal('hide');
...
$(document).on('click', '.requestProdiuctModal', function(e) {
    $('#requestProdiuctModal').modal('hide');
});
...
$(document).on('change', '#selKindOfProduct', function(e) {
    var selectedKind = $(this).val();

    $.ajax({
        type: "POST",
        url: "app/action.php",
        data: { selectedKind: selectedKind },
        success: function(response) {
            console.log(response);
            $('#selectD-noneRemove').removeClass('d-none');
            $('#selProductKind').html(response);
        }
    });
});

$(document).on('click', '#increseNum', function(e) {
    var input = $('.numericInput').val();
    var intInput = parseInt(input);
    var num = intInput + 1;
    if (num >= 999) {
        num = 999;
        $('.numericInput').val(num);
    } else {
        $('.numericInput').val(num);
    }
});

$(document).on('click', '#decriseNum', function(e) {
    var input = $('.numericInput').val();
    var intInput = parseInt(input);
    var num = intInput - 1;
    if (num <= 1) {
...
$(document).on('click', '#makeOrderProduct', function(e) {
    $('#requestProdiuctModal').modal('show');
});

$(document).on('click', '#viewRequestedProduct', function(e) {
    $('#viewRequestedProductModal').modal('show');
});

$(document).on('click', '#viewRequestedProductModalClose', function(e) {
    $('#viewRequestedProductModal').modal('hide');
});
...
$(document).on('click', '#exRotate', function(e) {
    ...
    $('#werHbtn').removeClass('box-btn-active');
    $('#chatBtn').removeClass('box-btn-active');
    $('.section1').removeClass('d-none');
    $('.section1-1').addClass('d-none');
    $('.section1-2').addClass('d-none');
    $('.section2').removeClass('d-none');
